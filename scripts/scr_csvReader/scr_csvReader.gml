// All this code is by the gmlscripts.com user Juju Adams.
// Modified to work in GameMaker v2023.4.0.113 by Miriam Logan.
...
//  @jujuadams
function ReadCSV(_filename, _force_strings = false, _cell_delimiter = chr(44), _string_delimiter = chr(34), _newline_alt = false)
{
	// chr 44 is comma
...
// This next method is also from gmlscripts.com, but modified by Miriam Logan.
...
function dsGridDraw(grid, x, y)
{
    var w = ds_grid_width(grid);
    var h = ds_grid_height(grid);
 
    var M = ds_grid_get_max(grid, 1, 1, w-1, h-1);
    var m = ds_grid_get_min(grid, 1, 1, w-1, h-1);
